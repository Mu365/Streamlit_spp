...
# Display 2D plot when CSV file is uploaded
if os.path.exists('dataset.csv'):
    df9 = pd.read_csv('dataset.csv', index_col=None)
    data = np.array(df9)
    ...
    ax.set_title('2D Foot Shape Visualization', fontsize=16)
    ...
    plt.tight_layout()
    st.pyplot(plt)
# Display 3D plot when CSV file is uploaded
if os.path.exists('dataset.csv'):
    df9 = pd.read_csv('dataset.csv', index_col=None)
    data = np.array(df9)
    ...
    layout = go.Layout(
        title='3D Foot Shape Visualization',
        scene=dict(
